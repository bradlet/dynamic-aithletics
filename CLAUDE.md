...
- **Week start**: Layout is always Sunday-first — standard `Calendar.current` week for grids, navigation, and repeating exercises (see `Date+Week.swift`). Weekly *aggregation* (mileage buckets, "This Week" stats) honors the user's `AppConfiguration.weekStartDay` (1=Sun…7=Sat), injected via `@Environment(\.weekStartDay)` and threaded into `WorkoutAggregations` as `firstWeekday`.
